...
<div class="p-10 mt-5 bg-white rounded-lg">
	<form action="{{ route('rates.update',$rate->id)  }}" method="POST">
	    ...
	    <div class="mt-5 font-bold">
	    	...
	    	<input type="text" name="payrate" value="{{ $rate->pay_rate }}" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1 rounded-full pl-5">
	    	@error('payrate')
	    	   <div class="text-red-700 ml-5 mb-1 font-medium">{{ $message }}</div>
	    	@enderror
	    </div>
	    ...
	    <div class="mt-5 font-bold">
	    	...
	    	<input type="text" name="billrate" value="{{ $rate->bill_rate }}" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1 rounded-full pl-5">
	    	@error('billrate')
	    	   <div class="text-red-700 ml-5 mb-1 font-medium">{{ $message }}</div>
	    	@enderror
	    </div>
